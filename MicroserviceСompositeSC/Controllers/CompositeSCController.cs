using MicroserviceСompositeSC.Contracts;
...
{
    private readonly string _studentServiceAddress = "https://localhost:7166/api/students";
    private readonly string _courseServiceAddress = "https://localhost:7105/api/course";
    ...
    public async Task<List<Student>> GetStudentsByGroupAsync(string groupName)
    {
        ...
        using (HttpClient client = new HttpClient(clientHandler))
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return students.Where(student => student.GroupName == groupName)
                    .ToList();
            }
        }
        return null;
